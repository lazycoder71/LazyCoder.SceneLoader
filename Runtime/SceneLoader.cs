...
namespace LazyCoder.SceneLoader
{
    public class SceneLoader : MonoSingleton<SceneLoader>
    {
        ...
        private async UniTaskVoid LoadAsync(AsyncOperation asyncOperation, CancellationToken cancellationToken)
        {
            ...
            await UniTask.WaitForSeconds(_fadeInDuration, true, cancellationToken: cancellationToken);

            // Fade in end
            _eventFadeInEnd?.Invoke();

            // Wait for scene load complete or min duration passed
            await WaitForSceneLoadedOrMinDuration(asyncOperation, cancellationToken: cancellationToken);

            // Fade out start
            _eventFadeOutStart?.Invoke();

            // Wait for fade out duration
            await UniTask.WaitForSeconds(_fadeOutDuration, true, cancellationToken: cancellationToken);
        ...
        private async UniTask WaitForSceneLoadedOrMinDuration(AsyncOperation handle, CancellationToken cancellationToken)
        {
        ...
        public void Load(string sceneName)
        {
            ...
            LoadAsync(asyncOperation, this.GetCancellationTokenOnDestroy()).Forget();
        }
